...
How It Works
Each user opens the app and gets a unique Peer ID from the PeerJS server
User A shares their Peer ID with User B
User B pastes the ID and clicks Connect
PeerJS handles the SDP offer/answer signaling via its cloud server
Once connected, all media (video/audio) and chat data flow directly P2P via WebRTC — no server involved
---
🚀 Deployment
...
---
🔧 How to Use
Open the app in a browser (two tabs or two different devices)
Copy your Jam ID (shown in the top bar)
Paste the other user's Jam ID into the input field
Click Connect
Both users will now share live video, audio, and can chat in real-time
Use the 🎤 and 📷 buttons to mute/unmute or toggle the camera
---
⚠️ Known Limitations
